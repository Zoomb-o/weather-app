...
function WindChart({ forecast }) {
    ...
    datasets: [
      {
        ...
        data: items.map((item) => Math.round(item.wind.speed * 3.6)),
        borderColor: '#34d399',
      ...
      {
        label: 'Széllökés (km/h)',
        data: items.map((item) => Math.round((item.wind.gust || 0) * 3.6)),
        borderColor: '#6ee7b7',
  ...
  const options = {
    ...
    scales: {
      ...
      y: {
        ticks: { color: 'white', callback: (val) => val + ' km/h' },
        grid: { color: 'rgba(255,255,255,0.1)' }
      }
    }
  }
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 mt-6">
      <div className="bg-white/20 backdrop-blur-md rounded-3xl border border-white/30 p-6 text-white">
        <h3 className="text-lg font-semibold mb-2">💨 Szélsebesség - Következő 24 Óra</h3>
        <p className="text-white/70 text-sm mb-4">Szélsebesség és széllökések km/h-ban</p>
