...
function TemperatureChart({ forecast }) {
  ...
  const data = {
    ...
    datasets: [
      {
        label: 'Hőmérséklet °C',
        data: items.map((item) => Math.round(item.main.temp)),
        borderColor: '#f97316',
      ...
      {
        label: 'Hőérzet °C',
        data: items.map((item) => Math.round(item.main.feels_like)),
        borderColor: '#facc15',
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 mt-6">
      <div className="bg-white/20 backdrop-blur-md rounded-3xl border border-white/30 p-6 text-white">
        <h3 className="text-lg font-semibold mb-2">🌡️ Hőmérséklet - Következő 24 Óra</h3>
        <p className="text-white/70 text-sm mb-4">Hőmérséklet és hőérzet °C-ban</p>
